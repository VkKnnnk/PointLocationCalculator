...
Метод решения задачи - [_**трассировка лучей**_](README.md#Описание_метода).
...
1. Чтобы найти точку пересечения двух прямых, необходимо составить систему уравнений вида:
    ...
    > где ***X***, ***Y*** - точка пересечения, ***K1*** - угловой коэфицент луча, ***K2*** - угловой коэфицент грани, ***B1*** - ордината луча, ***B2*** - ордината грани.
...
5. Чтобы найти ***X*** и ***Y***, необходимо знать, чему равны ***K1*** и ***B1*** луча, и ***K2*** и ***B2*** грани. Т.к. мы задаем точки для вершины и указываем тестовую точку, с которой начинается луч, мы можем найти эти значения. Чтобы найти ***K1***, ***B1***, ***K2***, ***B2*** необходимо составить систему уравнений для двух точек. Преобразуем предыдущую систему уравнений:
    ...
    > где ***K*** - угловой коэфицент прямой, ***B*** - ордината прямой, ***X1***, ***Y1*** - координаты начальной точки, ***X2***, ***Y2*** - координаты конечной точки.
   ...
   Соответсвенно ***B*** равен: `B = Y1 - KX1 = Y2 - KX2`
...
5. Подставляем полученные значения в систему уравнений:
   ...
   Соответсвенно ***Y*** равен: `y = k1x + b1 = k2x + b2`

5. Точка пересечения грани и луча равна полученным значениям ***X*** и ***Y***, если точка входит в диапозон между точек грани и больше тестовой точки, то луч пересекает грань.
...
Кроме того, в программе использутся **паттерн Command**, который помогает отделить логику действия пользователя от его представления,
